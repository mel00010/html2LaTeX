 ...
 * @brief Holds the definition of class DetermineCharEncoding and many other
 ...
 *
 * Implements the algorithms for determining the character encoding of an HTML
 * document defined in the W3C HTML5 specification.
 */
class DetermineCharEncoding {
 ...
 public:
   ...
   *
   * Uses the algorithms for determining the character encoding of a document
   * defined in Section 8.2 of the W3C HTML5 specification.
   * @param input The input stream whose character encoding will be determined.
   * @return A ContentType object containing the character encoding and the
   * confidence in the result.
   * @exception std::istream::failure The input stream failed before enough data
   * could be read to make a guess.
   * @test HTML::Parse::TEST(HTML_Parse_DetermineCharEncoding,
   * determineCharEncoding)
   */
  ContentType determineCharEncoding(std::istream& input);

 protected:
  /**
   * @brief Tries to use Unicode Byte Order Markings to determine the content
   ...
   * @param input The input stream whose character encoding will be determined.
   * @return If successful, returns a ContentType object containing the
   * character encoding and with the confidence CERTAIN. If unsuccessful,
   * returns a ContentType object with the character encoding UNKNOWN and the
  ...
  /**
   * @brief Scans the document looking for information about the character
   * encoding.
   * @param input The input stream whose character encoding will be determined.
   * @return If successful, returns a ContentType object containing the
   * character encoding and with the confidence CERTAIN. If unsuccessful,
   ...
   * @param input The input stream whose character encoding will be determined.
   * @return A ContentType object with the character encoding UNKNOWN and the
   * confidence IRRELEVANT.
   * @exception std::istream::failure The input stream failed before enough data
   * could be read to make a guess.
   *
  ...
  /**
   * @brief Parses an HTML/XML meta tag to attempt to guess the character
   * encoding.
   *
   * Helper function for ContentType preScan(std::istream& input)
   * Called by preScan() if the tag starts with the case insensitive sequence of
   * ASCII characters '<meta'
   * @see ContentType preScan(std::istream&
   * input)getCharEncodingFromString_utf_16le
   * @param input The input stream whose character encoding will be determined.
   * @return If successful, returns a ContentType object containing the
   * character encoding and with the confidence CERTAIN. If unsuccessful,
   * returns a ContentType object with the character encoding UNKNOWN and the
  ...
  /**
   * @brief Reads from the input stream until the end of an HTML tag that starts
   * with a sequence of ASCII letters.
   *
   * Helper function for ContentType preScan(std::istream& input)
   * Called by preScan() if a tag starts with a sequence of ASCII letters other
   * than '<meta'.
   * @see ContentType preScan(std::istream& input)
   * @param input The input stream whose character encoding will be determined.
   * @return A ContentType object with the character encoding UNKNOWN and the
  ...
  /**
   * @brief Reads from the input stream until the end an HTML tag that starts
   * with the ASCII characters '!', '?', or '/'.
   *
   * Helper function for ContentType preScan(std::istream& input)
   ...
   * @param input The input stream whose character encoding will be determined.
   * @param swallowExceptions Boolean value determining whether the algorithm
   * should throw an exception if the input stream throws an exception or simply
   * return with the character encoding UNKNOWN and confidence IRRELEVANT.
   * Defaults to true.
   * @return An ASCIIAttribute object containing the name of the attribute and
   * its value.getCharEncodingFromString_utf8 If swallowExceptions is true and
   * the input stream throws an exception, then the function returns an
   ...
   */
  ASCIIAttribute getAttribute(std::istream& input,
                              bool swallowExceptions = true);

  /**
   * @brief Gets an HTML attribute value from the input stream.
   *
   * Helper function for ASCIIAttribute getAttribute(std::istream& input, bool
   * swallowExceptions = true)
   * @param input The input stream whose character encoding will be determined.
   * @param buf A reference to the buffer holding the last character read from
   * the input stream
   ...
   * Helper function for std::string getAttributeValue(std::istream& input,
   *Byte& buf) Called if the attribute value has is in quotes.
  ...
  /**
   * @brief Extracts an attribute value that is not in quotes from the input
   * stream.
   *
   * Helper function for std::string getAttributeValue(std::istream& input,
   * Byte& buf) Called if the attribute value is not in quotes.
   * @see std::string getAttributeValue(std::istream& input, Byte& buf)
   * @param input The input stream whose character encoding will be determined.
   * @param buf A reference to the buffer holding the last character read from
   * the input stream
   ...
   * @return A string containing the attribute's value.
   */
  std::string processingLoop(std::istream& input, Byte& buf,
                             std::string& value);

  /**
   * @brief Extracts a character encoding from a HTML/XML meta tag.
   * @param string The string containing the meta tag to extract the character
   * encoding from
   * @return If a character encoding is successfully extracted, the function
   * returns the character encoding. If no character encoding is extracted, the
   * function returns NULL_ENC.
   *
   * @test HTML::Parse::TEST(HTML_Parse_DetermineCharEncoding,
   * extractCharEncodingFromMetaTag)
   */
   ...
   *
   * The character encoding is extracted according to the definitions in the
   * table below. String 					| Character
   * Encoding
   * -------------------------| -------------
   ...
   *  utf-16					| UTF_16_LE
   *
   *  Any string not in the table above is assigned the character encoding of
   * NULL_ENC.
   *
   * @see HTMl::Parse::CharEncoding
   * @param input The string the character encoding will be attempted to be
   * extracted from.
   * @return If a character encoding is successfully recognized, the function
   * returns the character encoding. If the character encoding is unrecognized,
   * the function returns NULL_ENC.
   *
   * @test HTML::Parse::TEST(HTML_Parse_DetermineCharEncoding,
   * getCharEncodingFromString)
   */
