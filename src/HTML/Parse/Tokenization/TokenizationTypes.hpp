...
enum class TokenType { NO_TOKEN, DOCTYPE, START_TAG, END_TAG, COMMENT, CHARACTER, END_OF_FILE };
...
inline bool operator!=(const DOCTYPEToken& lhs, const DOCTYPEToken& rhs) { return !(lhs == rhs); }

class TagToken {
 public:
  enum class TagType { NONE, START, END };

 public:
  TagToken() : type(TagType::NONE), tag_name(U""), self_closing(false), attributes({}) {}
  explicit TagToken(const TagType& type, const std::u32string& tag_name = U"", const bool& self_closing = false,
...
inline bool operator!=(const StartTagToken& lhs, const StartTagToken& rhs) { return !(lhs == rhs); }
...
inline bool operator!=(const EndTagToken& lhs, const EndTagToken& rhs) { return !(lhs == rhs); }

class CharacterToken {
 public:
  CharacterToken(const char32_t& data = EOF32) : data(data) {}
  operator Token();
...
inline bool operator!=(const CharacterToken& lhs, const CharacterToken& rhs) { return !(lhs == rhs); }
...
inline bool operator!=(const CommentToken& lhs, const CommentToken& rhs) { return !(lhs == rhs); }
...
inline bool operator!=(const EOFToken& lhs, const EOFToken& rhs) { return !(lhs == rhs); }
...
inline bool operator!=(const NoToken& lhs, const NoToken& rhs) { return !(lhs == rhs); }

typedef std::variant<NoToken, DOCTYPEToken, StartTagToken, EndTagToken, CharacterToken, CommentToken, EOFToken> TokenVariant;

class Token {
 public:
  Token(const NoToken& token) : type(TokenType::NO_TOKEN), token(token) {}
  Token(const DOCTYPEToken& token) : type(TokenType::DOCTYPE), token(token) {}
  Token(const StartTagToken& token) : type(TokenType::START_TAG), token(token) {}
  Token(const EndTagToken& token) : type(TokenType::END_TAG), token(token) {}
  Token(const CommentToken& token) : type(TokenType::COMMENT), token(token) {}
  Token(const CharacterToken& token) : type(TokenType::CHARACTER), token(token) {}
  Token(const EOFToken& token) : type(TokenType::END_OF_FILE), token(token) {}
  Token(const Token& token) : type(token.type), token(token.token) {}
  Token(NoToken* token) : type(TokenType::NO_TOKEN), token(*token) {}
  Token(DOCTYPEToken* token) : type(TokenType::DOCTYPE), token(*token) {}
  Token(StartTagToken* token) : type(TokenType::START_TAG), token(*token) {}
  Token(EndTagToken* token) : type(TokenType::END_TAG), token(*token) {}
  Token(CommentToken* token) : type(TokenType::COMMENT), token(*token) {}
  Token(CharacterToken* token) : type(TokenType::CHARACTER), token(*token) {}
  Token(EOFToken* token) : type(TokenType::END_OF_FILE), token(*token) {}
  Token(const TokenType& type, const TokenVariant& token) : type(type), token(token) {}
  Token() : type(TokenType::NO_TOKEN), token(NoToken()) {}
...
inline DOCTYPEToken::operator Token() { return Token(*static_cast<DOCTYPEToken*>(this)); }
...
inline StartTagToken::operator Token() { return Token(this); }
inline EndTagToken::operator Token() { return Token(this); }
inline CharacterToken::operator Token() { return Token(this); }
inline CommentToken::operator Token() { return Token(this); }
inline EOFToken::operator Token() { return Token(this); }
inline NoToken::operator Token() { return Token(this); }
...
enum State {
  ...
  SCRIPT_DATA_DOUBLE_ESCAPED_LESS_THAN_SIGN,  //!< SCRIPT_DATA_DOUBLE_ESCAPED_LESS_THAN_SIGN @brief Section 8.2.4.32 in the W3C HTML5
                                              //!< specification
  ...
  CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE,     //!< CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE @brief Section 8.2.4.41 in the W3C HTML5
                                              //!< specification
  ...
  DOCTYPE_PUBLIC_IDENTIFIER_DOUBLE_QUOTED,    //!< DOCTYPE_PUBLIC_IDENTIFIER_DOUBLE_QUOTED @brief Section 8.2.4.58 in the W3C HTML5
                                              //!< specification
  DOCTYPE_PUBLIC_IDENTIFIER_SINGLE_QUOTED,    //!< DOCTYPE_PUBLIC_IDENTIFIER_SINGLE_QUOTED @brief Section 8.2.4.59 in the W3C HTML5
                                              //!< specification
  AFTER_DOCTYPE_PUBLIC_IDENTIFIER,            //!< AFTER_DOCTYPE_PUBLIC_IDENTIFIER @brief Section 8.2.4.60 in the W3C HTML5 specification
  BETWEEN_DOCTYPE_PUBLIC_AND_SYSTEM_IDENTIFIERS,  //!< BETWEEN_DOCTYPE_PUBLIC_AND_SYSTEM_IDENTIFIERS @brief Section 8.2.4.61 in the W3C
                                                  //!< HTML5 specification
  ...
  DOCTYPE_SYSTEM_IDENTIFIER_DOUBLE_QUOTED,  //!< DOCTYPE_SYSTEM_IDENTIFIER_DOUBLE_QUOTED @brief Section 8.2.4.64 in the W3C HTML5
                                            //!< specification
  DOCTYPE_SYSTEM_IDENTIFIER_SINGLE_QUOTED,  //!< DOCTYPE_SYSTEM_IDENTIFIER_SINGLE_QUOTED @brief Section 8.2.4.65 in the W3C HTML5
                                            //!< specification
